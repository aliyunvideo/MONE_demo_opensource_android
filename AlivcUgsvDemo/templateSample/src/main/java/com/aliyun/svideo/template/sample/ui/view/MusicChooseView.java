...
import com.aliyun.svideo.template.sample.ui.adapter.MusicAdapter;
...
public class MusicChooseView extends LinearLayout implements View.OnClickListener {
    ...
    protected void onAttachedToWindow() {
        ...
        if (isShowed && mCacheMusic != null && mMusicAdapter != null) {
            ...
            mRecyclerView.scrollToPosition(mCachePosition);
    ...
    private void initView() {
        LayoutInflater.from(getContext()).inflate(R.layout.alivc_music_view_chooser_music2, this, true);
        findViewById(R.id.ugsv_music_confirm).setOnClickListener(this);
        findViewById(R.id.ugsv_music_close).setOnClickListener(this);
        mRecyclerView = findViewById(R.id.aliyun_music_list);
        ...
        if (mMusicAdapter == null) {
            ...
            mMusicAdapter.setOnMusicOperaCallback(new MusicAdapter.OnMusicOperaCallback() {
                            ...
                            public void onProgress(int downloadId, long soFarBytes, long totalBytes, long speed,
                                ...
                                if (!isLocalMusic) {
                                    mMusicAdapter.updateProcess(
                                        (MusicAdapter.MusicViewHolder) mRecyclerView
    ...
    private void onMusicSelected(MusicFileBean musicFileBean, int position) {
        ...
        if (musicSelectListener != null) {
            ...
            musicSelectListener.onMusicSelect(musicFileBean);
            //缓存选择的值
    ...
    public void onClick(View v) {
        if(v.getId() == R.id.ugsv_music_close){
            if(musicSelectListener != null) musicSelectListener.onMusicPanelClose(false);
        }else if(v.getId() == R.id.ugsv_music_confirm){
            if(musicSelectListener != null) musicSelectListener.onMusicPanelClose(true);
        }
    }
