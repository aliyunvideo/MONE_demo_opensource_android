...
public class MusicWaveView extends View {
    ...
    private static final float MIN_WAVE_RATE = 0.1f;
    private static final float MAX_FADE_WAVE_RATE = 0.3f;
    ...
    private void init(Context context) {
        mWaveHeight = DensityUtil.dip2px(context, 16);
        mScreenWidth = context.getResources().getDisplayMetrics().widthPixels;
        mSelectBgWidth = DensityUtil.dip2px(context, 124);
    ...
    private void generateWaveArray() {
        ...
        int fade = (int) (count * 0.1);
        for(int i = 0; i < fade; i++) {
            mWaveArray[i] = random.nextFloat();
            if (mWaveArray[i] >  MAX_FADE_WAVE_RATE) {
                mWaveArray[i] -= MAX_FADE_WAVE_RATE;
            }
            if (mWaveArray[i] <  MIN_WAVE_RATE) {
                mWaveArray[i] += MIN_WAVE_RATE;
            }
        }
        for (int i = fade ; i < count - fade ; i++) {
            mWaveArray[i] = random.nextFloat();
            if (mWaveArray[i] <  MIN_WAVE_RATE) {
                mWaveArray[i] += MIN_WAVE_RATE;
            }
        }
        for(int i = count - fade; i < count; i++) {
            mWaveArray[i] = random.nextFloat();
            if (mWaveArray[i] >  MAX_FADE_WAVE_RATE) {
                mWaveArray[i] -= MAX_FADE_WAVE_RATE;
            }
