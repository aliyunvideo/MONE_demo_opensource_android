...
public abstract class BaseScrollPickerView<T> extends View {
     ...
     * 中间选中item的游标图标
     */
    private Drawable mCursorIcon = null;
    /**
     * 中间选中item的游标大小
     */
    private int mCursorSize = dip2px(4);
    ...
    private void setCursorIconBounds() {
        mCursorIcon.setBounds(
                mCenterX + (mItemWidth >> 1) - mCursorSize / 2,
                mCenterY + mItemHeight - mCursorSize,
                mCenterX + (mItemWidth >> 1) + mCursorSize / 2,
     ...
     */
    public void setCursorIcon(Drawable iconDrawable) {
        mCursorIcon = iconDrawable;
        setCursorIconBounds();
        invalidate();
    }
     ...
     * @param iconColor 颜色值
     */
    public void setCursorIcon(int iconColor) {
        GradientDrawable icon = new GradientDrawable();
        icon.setColor(iconColor);
        icon.setCornerRadius((float) mCursorSize * 0.5f);
        setCursorIcon(icon);
    }
