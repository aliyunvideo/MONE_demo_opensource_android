...
public class FileDownloaderDBCipherController implements IFileDownloaderDBController {
    private FileDownloaderDBCipherOpenHelper mDBHelper;
    private String mTableName;
    private String mCk;

    public FileDownloaderDBCipherController(FileDownloaderDBCipherOpenHelper dBHelper, String tableName, String ck) {
        mDBHelper = dBHelper;
    ...
    public SparseArray<FileDownloaderModel> getAllTasks() {
        SQLiteDatabase sqliteDatabase = mDBHelper.getReadableDatabase(mCk);
    ...
    public synchronized FileDownloaderModel addTask(FileDownloaderModel downloaderModel) {
        ...
        SQLiteDatabase sqliteDatabase = mDBHelper.getWritableDatabase(mCk);
    ...
    public synchronized boolean insertDb(FileDownloaderModel model, HashMap<String, String> hashMap) {
        ...
        boolean succeed = false;
        //查询是否已经存在
        List<FileDownloaderModel> list = null;
        if (hashMap != null) {
            list = getResourceByFiled(hashMap);
        }
...
//        int type = c.getInt(c.getColumnIndex(FileDownloaderModel.TYPE));
    ...
    public FileDownloaderDBCipherOpenHelper getDBHelper() {
        return mDBHelper;
    }
}
