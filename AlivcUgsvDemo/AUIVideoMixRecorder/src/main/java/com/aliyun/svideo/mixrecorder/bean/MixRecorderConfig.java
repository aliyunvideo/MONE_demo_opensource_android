 ...
 * 录制参数配置
 */
public class MixRecorderConfig {
    private static MixRecorderConfig INSTANCE = null;
    private static Object MUTEX = new Object();
    public static MixRecorderConfig getInstance(){
        if(INSTANCE == null){
            synchronized (MUTEX){
                if(INSTANCE == null){
                    INSTANCE = new MixRecorderConfig();
                }
            }
        }
        return INSTANCE;
    ...
    public static List<Pair<VideoDisplayParam, VideoDisplayParam>> LAYOUT_OPTIONS = new ArrayList<>();
    static {
        LAYOUT_OPTIONS.add(
                new Pair(
                        new VideoDisplayParam.Builder().setmCenterX(0.25f).setmCenterY(0.5f).setmWidthRatio(0.5f).setmHeightRatio(1.0f).build(),
                        new VideoDisplayParam.Builder().setmCenterX(0.75f).setmCenterY(0.5f).setmWidthRatio(0.5f).setmHeightRatio(1.0f).build()
                        )
        );
        LAYOUT_OPTIONS.add(
                new Pair(
                        new VideoDisplayParam.Builder().setmCenterX(0.5f).setmCenterY(0.25f).setmWidthRatio(1.0f).setmHeightRatio(0.5f).build(),
                        new VideoDisplayParam.Builder().setmCenterX(0.5f).setmCenterY(0.75f).setmWidthRatio(1.0f).setmHeightRatio(0.5f).build()
                )
        );
        LAYOUT_OPTIONS.add(
                new Pair(
                        new VideoDisplayParam.Builder().setmCenterX(0.15f).setmCenterY(0.15f).setmWidthRatio(0.3f).setmHeightRatio(0.3f).layoutLevel(2).build(),
                        new VideoDisplayParam.Builder().setmCenterX(0.5f).setmCenterY(0.5f).setmWidthRatio(1.0f).setmHeightRatio(1.0f).layoutLevel(1).build()
                )
        );
    }

    private MixRecorderConfig() {
        ...
        this.mRatioMode = RATIO_MODE_1_1;
        this.mGop = DEFAULT_VALUE_GOP;
        this.mFPS = DEFAULT_VALUE_FRAME;
        this.mVideoQuality = VideoQuality.HD;
        this.mVideoCodec = VideoCodecs.H264_HARDWARE;
        this.mRenderingMode = BeautySDKType.QUEEN;
        this.mIsUseFlip = false;
        this.mMixAudioSourceType = MixAudioSourceType.Original;
        this.mMixBackgroundColor = Color.BLACK;
        this.mPlayDisplayParam = LAYOUT_OPTIONS.get(0).first;
        this.mRecordDisplayParam = LAYOUT_OPTIONS.get(0).second;
    }
    /**
     * 获取拍摄视频宽度
     * @return
    ...
    public BeautySDKType getRenderingMode() {
    ...
    public Pair<VideoDisplayParam, VideoDisplayParam> getLayout(LayoutOption option){
        int index = 0;
        switch (option){
            case HORIZONTAL:
                index = 0;
        ...
        return new Pair<>(
                new VideoDisplayParam.Builder()
                .setmCenterX(LAYOUT_OPTIONS.get(index).first.getCenterX())
                .setmCenterY(LAYOUT_OPTIONS.get(index).first.getCenterY())
                .setmWidthRatio(LAYOUT_OPTIONS.get(index).first.getWidthRatio())
                .setmHeightRatio(LAYOUT_OPTIONS.get(index).first.getHeightRatio())
                .layoutLevel(LAYOUT_OPTIONS.get(index).first.getLayoutLevel())
                .build(),
        new VideoDisplayParam.Builder()
                .setmCenterX(LAYOUT_OPTIONS.get(index).second.getCenterX())
                .setmCenterY(LAYOUT_OPTIONS.get(index).second.getCenterY())
                .setmWidthRatio(LAYOUT_OPTIONS.get(index).second.getWidthRatio())
                .setmHeightRatio(LAYOUT_OPTIONS.get(index).second.getHeightRatio())
                .layoutLevel(LAYOUT_OPTIONS.get(index).second.getLayoutLevel())
                .build());
    }

    public static class Builder {
        private MixRecorderConfig mParam = new MixRecorderConfig();
        ...
        public MixRecorderConfig build() {
            return this.mParam;
        }
    }
}
